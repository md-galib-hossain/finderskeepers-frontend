...
const ViewFullRecentPostCardModal = ({ lostItem, open, setOpen,user }: TOpenProps) => {
  ...
  const handleFormSubmit = async (values: FieldValues) => {
    ...
    try {
      const updatedData = await modifyUpdateFormData(values,lostItem)
      updatedData.id = lostItem?.id
      const res = await updateLostItem(updatedData).unwrap();
      console.log(res);
      if (res?.id) {
    ...
    <>
      ...
      >
        ...
        >
          ...
          {!editable && lostItem.lostItemStatus === "NOTFOUND" && lostItem?.user?.id === user?.id &&
...
export default ViewFullRecentPostCardModal;
